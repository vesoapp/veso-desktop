...
then
    (
        get_resource_version dannymichel/veso-web
...
then
    if [[ "$(get_resource_version dannymichel/veso-web)" != "$(cat .last_wc_version)" ]]
    then
        update_web_client="yes"
    fi
fi
...
then
    ...
    wc_version=$(get_resource_version dannymichel/veso-web)
    download_compat dist.zip "https://github.com/dannymichel/veso-web/releases/download/$wc_version/dist.zip" "wc"
    if [[ "$DOWNLOAD_ONLY" != "1" ]]
    then
